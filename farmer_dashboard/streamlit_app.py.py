...
# Helper: convert Plotly figure to PNG BytesIO using plotly.io (kaleido)
def fig_to_bytes(fig_obj, format="png", scale=2):
    """
    Convert Plotly figure to a BytesIO buffer (png bytes).
    Requires 'kaleido' installed (add to requirements.txt).
    """
    img_bytes = pio.to_image(fig_obj, format=format, scale=scale)
...
with col1:
    ...
    if normalize and population_for_norm > 0:
        scale = 100000.0 / float(population_for_norm)
        ts_display['deaths'] = ts_display['deaths'] * scale
        y_title = 'Deaths per 100k'
    else:
        y_title = 'Deaths'

    # Plot normalized or raw series (use ts_display so y-axis matches normalization)
    fig = px.line(ts_display, x='date', y='deaths', title='Monthly deaths over time')
    fig.update_yaxes(title=y_title)
    st.plotly_chart(fig, use_container_width=True)

    # download button for timeseries (use BytesIO)
    st.download_button(
        label='Download timeseries PNG',
        data=fig_to_bytes(fig),
        file_name='timeseries.png',
        mime='image/png'
    )
    ...
    st.download_button(
        label='Download stacked area PNG',
        data=fig_to_bytes(fig2),
        file_name='stacked_area.png',
        mime='image/png'
    )
...
fig3 = px.imshow(
    heat_pivot,
    labels=dict(x='Month', y='Year', color='Deaths'),
...
st.download_button(
    label='Download cause pie PNG',
    data=fig_to_bytes(fig4),
    file_name='cause_pie.png',
    mime='image/png'
)
...
st.download_button(
    ...
    data=csv_b,
    file_name='farmer_deaths_filtered.csv',
    mime='text/csv'
)
...
st.download_button(
    label='Download yearly PNG',
    data=fig_to_bytes(fig_year),
    file_name='yearly.png',
    mime='image/png'
)
...
st.download_button(
    label='Download month PNG',
    data=fig_to_bytes(fig_month),
    file_name='by_month.png',
    mime='image/png'
)
